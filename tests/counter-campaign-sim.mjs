...
// Usage: node --experimental-loader ./tests/loader.mjs ./tests/counter-campaign-sim.mjs [sorties] [dodge] [seed] [hard]
//   hard: 1 also flies the lowest Hard stage not yet cleared (Hard opens once the normal stage is cleared) each sortie.
...
import { STAGES, hardRec } from '@last-orbit/data/counter.js';
...
const N = Number(process.argv[2] || 40), dodge = Number(process.argv[3] ?? 1), seed0 = Number(process.argv[4] || 1), withHard = process.argv[5] === '1';
...
const rows = [], firstClear = {}, tries = {}, hardClear = {}, hardTries = {}; let lastFailed = false;
for (let n = 1; n <= N; n++) {
  ...
  let hd = '';
  if (withHard && c.unlocked) {
    const sg = STAGES.find((x) => c.stars[x.n] && !c.hard[x.n]);
    if (sg) { const power = powerRating(); hardTries[sg.n] = (hardTries[sg.n] || 0) + 1; const r = fly({ counter: sg.n, hard: true, seed: seed0 * 9000 + n }, true).counter; spend();
      if (r.cleared && !hardClear[sg.n]) hardClear[sg.n] = { sortie: n, tries: hardTries[sg.n], power };
      hd = `H${sg.n} ${r.cleared ? r.stars + '*' : 'fail ' + Math.round(r.killed * 100) + '%'}`; }
  }
  rows.push({ n, wave: s.wave, hard: hd, workshop: Object.values(G.state.workshop).reduce((a, b) => a + b, 0), mastery: G.state.mastery[G.state.ship]?.level || 1, power: powerRating(), cores: c.cores, counter: ca });
}
console.table(rows);
console.log(`dodge ${dodge} seed ${seed0}: unlocked at sortie ${rows.find((r) => r.counter)?.n ?? '-'}`);
if (withHard) for (const sg of STAGES) { const f = hardClear[sg.n]; console.log(`HARD stage ${sg.n} (rec ${sg.rec + hardRec(sg.n)}): ${f ? `cleared sortie ${f.sortie}, attempt ${f.tries}, power ${f.power}` : `not cleared (${hardTries[sg.n] || 0} attempts)`}`); }
