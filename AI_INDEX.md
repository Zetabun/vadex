...
| Resuming a sortie after the app closes (the checkpoint, the offer at launch, resume or end) | `modules/progression/run.js` (checkpoint, resumeOffer, resumeInfo), `modules/main.js` (adopt at boot, offerResume, resumeSortie, endResumed), `modules/ui/overlays.js` (showResume), `tests/resume-regression.mjs` |
| The 3D rooms aboard (the Room base, doors, taking a room down; the renderer keeps the last three built) | `modules/rendering/room.js` (Room, dispose, disposeScene), `modules/rendering/renderer.js` (room, keepRoom, dropRoom, ROOM_KEEP) |
| The Cipher (fragments, the tuner, the Origin sector, the Cipher boss and its voices, the Keeper rewards) | `modules/data/cipher.js`, `modules/progression/cipher.js`, `modules/rendering/cipher.js` (CipherRoom), `modules/ui/overlays.js` (showTune), `modules/ui/hangar.js` (cipherExhibit), `modules/data/sectors.js` (hidden.origin), `modules/data/routes.js` (SIGNAL), `modules/data/bosses.js` + `modules/combat/bosses.js` (phase mask/move), `tests/cipher-regression.mjs`, `tests/cipher-probe.mjs` |
