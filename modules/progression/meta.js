...
export function selectShip(id) { if (!G.state.unlocked.ships[id] || G.state.run || G.state.fleet?.out?.some((o) => o?.ship === id) || G.state.fleet?.damage?.[id] || (G.state.refitting?.ship === id && G.state.refitting.since != null)) return false; /* a ship out on an expedition (progression/fleet.js) stays out; one home damaged waits for repairs */ G.state.ship = id; recalc(); bus.emit('shipSelected', id); return true; }
