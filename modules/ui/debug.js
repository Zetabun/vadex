...
function runScene(scene, hooks, ui) {
  ...
  if (name === 'control') { const n = Math.min(6, +arg || 3), held = arg3 == null ? Math.max(0, n - 2) : +arg3; st.pilot.name = 'Adam'; st.seen.callsign = true; st.counter.unlocked = true; for (let k = 1; k <= n; k++) st.counter.stars[k] = st.counter.stars[k] || 2; const bare = arg4 === 'bare'; /* control:n:view:held:bare: nothing built yet */ if (!bare) { st.counter.tech.x_alloy = 1; st.counter.tech.x_phase = 1; }
    WORKSHOP.forEach((u, i) => { st.workshop[u.id] = bare ? 0 : Math.round(u.max * Math.min(1, Math.max(0, 0.9 - (i % 5) * 0.22))); }); if (bare) st.stationPeak = {}; st.siege = { stars: {}, best: {}, won: {}, wins: held }; for (let k = 1; k <= held; k++) { st.siege.stars[k] = 1 + (k % 3); st.siege.best[k] = 20000 + k * 7000; st.siege.won[k] = 1; }
