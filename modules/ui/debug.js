...
function runScene(scene, hooks, ui) {
  ...
  // records:global[:yday|all|instead]: the global boards (point the game at a local server with &api=http://127.0.0.1:8787;
  // instead: the callsign was reserved for someone else, so the boards show the pilot as Pilot)
  if (name === 'records' && arg === 'global') { if (arg2 === 'instead') { st.pilot.name = 'Zetabun'; Object.assign(st.global, { shownAs: 'Pilot', sentAs: 'Zetabun', tag: 'K7QM' }); } for (const r of ROOMS_ABOARD) (st.seen.offered ||= {})[r.id] = true; /* no room offers over the boards */ hooks.toHangar('records'); setTimeout(() => { [...document.querySelectorAll('.gl-seg button')].find((x) => x.textContent === 'Global')?.click(); if (arg2 && arg2 !== 'instead') setTimeout(() => [...document.querySelectorAll('.gl-chip')].find((x) => x.textContent.toLowerCase().startsWith(arg2))?.click(), 200); }, 400); return; }
  ...
  // breach[:<strikes>]: a sortie with the line already breached that many times this sector (default 1; 2: the last
  // chance, the markers pulsing red)
  else if (name === 'breach') { const run = st.run; run.offer = null; run.pendingLevels = 0; ui.closeOverlays(); run.strikes = +(arg || 1); }
