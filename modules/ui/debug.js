...
function runScene(scene, hooks, ui) {
  ...
  if (name === 'gunner') { st.pilot.name = 'Adam'; st.seen.callsign = true; st.counter.unlocked = true; st.counter.stars[1] = 2; WORKSHOP.forEach((u, i) => { st.workshop[u.id] = Math.round(u.max * Math.min(1, Math.max(0, 0.6 - (i % 5) * 0.13))); }); st.seen.control = true; recalc(); hooks.toHangar('control'); setTimeout(() => hooks.toHangar('gunner'), 400); return; }
