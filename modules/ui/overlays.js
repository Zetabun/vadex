...
import { observatoryOpen, OBSERVATORY_RANK } from '@last-orbit/data/observatory.js';
...
export function createOverlays(layer, hooks) {
  ...
  function showDebrief(s) {
    ...
    const el = h('div.modal.debrief', { role: 'dialog', 'aria-label': 'Sortie debrief' },
      ...
      s.cipher ? h('div.pilot-row.void-row.cipher-row', h('span', s.cipher.first ? 'The Cipher beaten: the signal is silent' : 'The Cipher beaten again'), h('b', `+${s.cipher.bp} Blueprints` + (s.cipher.first ? ' · Keeper paint and title' : ''))) : s.origin ? h('div.pilot-row.void-row.cipher-row', h('span', 'Reached the Origin'), h('b', 'The Cipher still waits')) : null,
      s.fragments ? h('div.pilot-row.void-row', h('span', `Signal fragment${s.fragments > 1 ? 's' : ''} found: ${s.fragments}`), h('b', (G.state.prestige?.level || 0) >= 10 ? 'Decode in the Cipher room' : 'Kept until the crown can read them')) : null,
