 ...
 *  does). seen: its first-visit flag in state.seen. for: what it is for, in a line. door: the way in from the room via.
 *  lock: what a tap says while it is shut. intro: what it says on the first visit (or when it offers the way aboard).
 *  wing: an Overhaul rank that adds to it, and what it says then. color: its accent. */
export const ROOMS_ABOARD = [
  { id: 'garden', name: 'Greenhouse', icon: 'garden', seen: 'garden', open: gardenOpen, announce: true, when: 'Opens a few sorties in, once you have cleared sector 1', color: 0x7ddc6f,
  ...
  { id: 'hall', name: 'Trophy Hall', icon: 'awards', seen: 'hall', rank: HALL_RANK, color: 0xffc857,
    for: 'The bosses you capture, and your hunting record', via: 'deck', door: 'the Command Deck\'s left-hand door',
  ...
  { id: 'comms', name: 'Comms room', icon: 'comms', seen: 'commsRoom', rank: COMMS_RANK, color: 0x6dffc8,
    for: 'Three bounties a day, for salvage and a Blueprint', via: 'hall', door: 'the Trophy Hall\'s right-hand door',
  ...
  { id: 'quarters', name: 'Pilot\'s quarters', icon: 'home', seen: 'quarters', rank: QUARTERS_RANK, color: 0xffb070,
    for: `Rest once a day for +${Math.round(REST_BONUS * 100)}% salvage on your next sortie`, via: 'hall', door: 'the Trophy Hall\'s left-hand door',
  ...
  { id: 'observatory', name: 'Observatory', icon: 'observatory', seen: 'observatory', rank: OBSERVATORY_RANK, color: 0xd9a441,
    for: 'Chart the Deep Void, past wave 60, for Blueprints', via: 'quarters', door: 'the right-hand door in your quarters',
  ...
  { id: 'yard', name: 'Shipyard', icon: 'yard', seen: 'shipyard', rank: YARD_RANK, color: 0xffc93c,
    for: 'Build the Chimera, a sixth ship', via: 'observatory', door: 'the Observatory\'s right-hand door',
...
/** Where to find a room: its door, if the room that door is in is open; otherwise the station on the Launch screen. */
export const roomWhere = (r, state) => (r.door && (!r.via || roomOpen(ROOM_BY_ID[r.via], state)) ? `Its door is ${r.door}, or tap your station on the Launch screen.` : 'Tap your station on the Launch screen to go aboard any time.');
