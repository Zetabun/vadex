...
function keepsake(shape, mat, glow) {
  ...
  switch (shape) {
    ...
    case 'pin': for (const s of [-1, 1]) M(new THREE.BoxGeometry(0.1, 0.03, 0.012), s * 0.06, 0.11, 0, 0, 0, s * -0.18); M(new THREE.SphereGeometry(0.03, 12, 8), 0, 0.11, 0.004, 0, 0, 0, glow); break;
...
export class QuartersRoom extends Room {
  ...
  furnish() {
    ...
    // the bunk, under the window: frame, mattress, blanket, pillow; a reading lamp on the wall by its head, your name
    // over the window
    const bl = BUNK.x1 - BUNK.x0, bx = (BUNK.x0 + BUNK.x1) / 2, bz = FRONT + 0.18 + BUNK.d / 2, bunk = new THREE.Group(); S.add(bunk);
    box(bl, 0.32, BUNK.d, bx, 0.2, bz, wood, bunk); box(bl - 0.06, 0.14, BUNK.d - 0.04, bx, 0.43, bz, Ph({ color: 0xe8e2d8, shininess: 10 }), bunk);
    this.blanket = box(bl * 0.62, 0.08, BUNK.d + 0.02, bx + bl * 0.19, 0.52, bz, Ph({ color: 0x3a5a8a, shininess: 8 }), bunk);
    box(0.36, 0.12, 0.62, BUNK.x0 + 0.3, 0.55, bz, Ph({ color: 0xf2eee6, shininess: 8 }), bunk);
    for (const x of [BUNK.x0 - 0.03, BUNK.x1 + 0.03]) box(0.06, 0.7, BUNK.d, x, 0.35, bz, wood, bunk);
    this.readLamp = new THREE.Mesh(new THREE.SphereGeometry(0.06, 12, 8), new THREE.MeshBasicMaterial({ color: 0xffd9a0 })); this.readLamp.position.set(BUNK.x0 - 0.1, 1.05, FRONT + 0.2); bunk.add(this.readLamp);
    ...
    box(SHELF.w, 1.95, 0.04, 0, 0.975, 0.02, dark, cab); this.shelfLights = [];
    for (const y of [0.1, 0.62, 1.14, 1.66]) { box(SHELF.w + 0.06, 0.04, 0.46, 0, y, 0.23, wood, cab); const l = box(SHELF.w - 0.1, 0.012, 0.02, 0, y + 0.47, 0.42, new THREE.MeshBasicMaterial({ color: 0xffb070 }), cab); this.shelfLights.push(l); }
    for (const s of [-1, 1]) box(0.05, 1.95, 0.46, s * (SHELF.w / 2 + 0.02), 0.975, 0.23, wood, cab);
    ...
    box(0.08, 0.1, 0.08, 0.45, 0.84, 0.42, Ph({ color: 0xd9dee8, shininess: 60 }), desk); // a mug
    ...
    this.rug = new THREE.Mesh(new THREE.CircleGeometry(1.25, 48), Ph({ color: 0x5a3a4a, shininess: 5 })); this.rug.rotation.x = -Math.PI / 2; this.rug.position.set(0, 0.012, -1.6); S.add(this.rug);
  ...
  sync(state) {
    ...
    KEEPSAKES.forEach((k, i) => {
      ...
      const o = keepsake(k.shape, mat, glow); o.position.set(-SHELF.w / 2 + 0.28 + col * ((SHELF.w - 0.56) / 3), 1.14 - row * 0.52 + 0.12, 0.24); o.scale.setScalar(1.25); o.userData.spin = i * 0.9; o.userData.has = has; this.pieces.add(o);
    });
  ...
  setMood(m) {
    ...
    this.stripMat.color.setHex(m.strip); this.coveMat.color.setHex(m.cove); for (const l of this.shelfLights) l.material.color.setHex(m.strip);
  ...
  update(dt) {
    ...
    if (this.sleepT > 0) { this.sleepT = Math.max(0, this.sleepT - dt); const k = Math.sin((1 - this.sleepT / 2.4) * Math.PI); for (const l of this.lamps) l.intensity = this.mood.lampI * (1 - k * 0.92); this.hemi.intensity = this.mood.hemi * (1 - k * 0.85); this.readLamp.visible = k < 0.5; }
