...
import { playSfx, lockTone } from '@last-orbit/audio/audio.js';
...
const GROW = { fighter: 0.3, bomber: 0.12, weak: 0.06 };
...
export class GunnerScene {
  ...
  start() {
    ...
    this.ms = { ammo: 0, reloadT: 0, target: null, lockT: 0, locked: false, beepT: 0, noLock: 0 };
    if (G.state) this.sync(G.state); lockTone(false);
  }
  sync(state) {
    this.station.sync(state); if (this.kit) return;
    this.kit = turretKit(state); this.st = turretStats(this.kit, this.picks); this.ms.ammo = this.st.missiles; this.picksDue = this.kit.freePicks; this.rerolls = this.kit.rerolls; this.beacon = this.kit.beacon;
  }
  ...
  choosePick(i) {
    ...
    const before = this.st.missiles; this.st = turretStats(this.kit, this.picks); this.ms.ammo += Math.max(0, this.st.missiles - before); m.now?.(this); this.pick = null; this.gap = 0.8; playSfx('buy', 0.8); return m;
  }
  ...
  damage(e, n) { if (!e.alive || e.kind === 'capital') return false; e.hp -= n; e.flash = 1; if (e.hp <= 0) { this.kill(e); return true; } return false; }
  kill(e) {
  ...
  burst(at, r, n, skip) { for (const e of this.enemies) if (e !== skip && e.alive && e.kind !== 'capital' && e.pos.distanceTo(at) < r + e.k.r) this.damage(e, n); }
  ...
  fireMissile() {
    ...
    this.scene.add(g); this.missiles.push({ g, p: from, v: fwd.multiplyScalar(70).add(new THREE.Vector3(0, -6, 0)), target: ms.target, life: 6, trailT: 0 });
    ms.ammo--; if (ms.reloadT <= 0) ms.reloadT = this.st.reload; ms.locked = false; ms.lockT = 0; ms.target = null; lockTone(false); playSfx('missile', 1); this.shake = 0.15; return true;
  }
  seek(dt, fwd, origin) {
    const ms = this.ms; if (ms.ammo < this.st.missiles) { ms.reloadT -= dt; if (ms.reloadT <= 0) { ms.ammo++; ms.reloadT = ms.ammo < this.st.missiles ? this.st.reload : 0; } }
    ...
    if (!ms.target && ms.ammo > 0) { let best = LOCK_CONE * (this.st.cone / 0.09); for (const e of this.enemies) { if (!e.alive || e.kind === 'capital' || e.kind === 'torpedo') continue; const a = ang(e); if (a < best) { best = a; ms.target = e; } } ms.lockT = 0; ms.beepT = 0; }
  ...
  update(dt) {
    ...
    this.locked = target; this.fireT -= fdt;
    if (target && fight && this.fireT <= 0) {
      this.fireT = this.st.fireEvery; const b = this.barrels[this.side = 1 - this.side]; b.kick = 1; b.flash.visible = true; b.flash.rotation.z = Math.random() * 6;
    ...
    for (let i = this.bullets.length - 1; i >= 0; i--) {
      ...
      if (hit) {
        ...
        if (!this.damage(hit, n0)) this.blast(u.p, 3, 0.15, this.cyanTex);
        if (this.st.splash) { this.burst(at, this.st.splash, n0 * 0.5, hit); this.blast(at, this.st.splash * 1.4, 0.3); }
    ...
    for (let i = this.missiles.length - 1; i >= 0; i--) {
      const m = this.missiles[i]; m.life -= fdt; if (!fdt) continue; const tg = m.target?.alive ? m.target : null;
      const speed = Math.min(MISSILE_SPEED, m.v.length() + 260 * dt);
      if (tg) { const want = this.lead(tg, m.p, speed).clone().sub(m.p).normalize().multiplyScalar(speed); m.v.lerp(want, Math.min(1, dt * 4.5)); } else m.v.setLength(speed);
  ...
  arc(at, n, skip) {
    ...
    for (const e of near) { const b = this.arcs.find((x) => !x.m.visible); if (b) { this.beam(b.m, at, e.pos); b.t = 0.15; } this.damage(e, n); }
  ...
  draw2d(ctx, W, H, pr) {
    ...
    for (const e of this.enemies) {
      ...
      if (e.kind === 'bomber') { ctx.fillStyle = col; ctx.font = '800 10px sans-serif'; ctx.textAlign = 'center'; ctx.fillText('BOMBER', s.x, s.y - r - 6); ctx.fillStyle = 'rgba(255,255,255,.2)'; ctx.fillRect(s.x - r, s.y + r + 4, 2 * r, 3); ctx.fillStyle = col; ctx.fillRect(s.x - r, s.y + r + 4, 2 * r * Math.max(0, e.hp / e.max), 3); }
  ...
  status() {
    ...
    return { hull: this.hull, shield: this.shield, wave: Math.max(0, this.wave) + 1, waves: WAVES.length, score: this.score, over: this.over, won: this.won, stars,
      ammo: ms.ammo, missiles: this.st?.missiles || 0, reload: ms.reloadT > 0 && this.st ? 1 - ms.reloadT / this.st.reload : 1, seeking: !!ms.target && !ms.locked, locked: ms.locked,
