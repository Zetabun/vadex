...
export class Renderer {
  ...
  /** The room aboard that is open (G.room: 'deck', 'control', 'hall', 'comms', 'quarters', 'observatory', 'yard' or 'gunner'), built the first time it is visited. */
  get room() { if (G.mode !== 'hangar' || !G.room) return null; const R = (this.rooms ||= {}); return (R[G.room] ||= G.room === 'control' ? new ControlRoom() : G.room === 'gunner' ? new GunnerScene() : G.room === 'hall' ? new HallRoom() : G.room === 'comms' ? new CommsRoom() : G.room === 'quarters' ? new QuartersRoom() : G.room === 'observatory' ? new ObservatoryRoom() : G.room === 'yard' ? new YardRoom() : new DeckRoom()); }
