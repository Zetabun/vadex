...
For browser verification, serve the root over HTTP. Confirm the page boots, Loadout opens across the viewport, empty slots have no icons, Back to game returns to combat, Arsenal's Go to Loadout opens the fitting screen, Wave 8 unlocks Skills, skill nodes can be inspected before purchase, deeper tiers open at Waves 12 and 20, and buying a rank updates available points. Tap a boon chip to inspect its effect and stacks. Check the skill map at 320px and 390px phone widths. Use `?debug=1` only in its isolated sandbox when accelerating progression.
